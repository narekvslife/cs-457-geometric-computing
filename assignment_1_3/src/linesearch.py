...
def evaluate_armijo_rule(f_x, f_x1, p, grad, c, alpha):
    ...
    """
    return f_x1 <= f_x + c * alpha * p.T @ grad
...
def backtracking_line_search(p, grad, x, theta, beta, c, f, *arg):
    ...
    objective_x = np.zeros((x.shape[0]//2, 3))
    objective_x[:, :2] = x.reshape((x.shape[0]//2, 2))

    f_x = f(objective_x, *arg)

    while True:
        objective_x1 = np.zeros((x.shape[0] // 2, 3))
        objective_x1[:, :2] = (x + alpha * p).reshape((x.shape[0]//2, 2))

        f_x1 = f(objective_x1, *arg)
        if evaluate_armijo_rule(f_x, f_x1, p, grad, c, alpha):
            return alpha
...
def gradient_descent_with_line_search(V, F, x_csl, w, obj_tol, theta, beta, c, iter):
    ...
    while(True):
        ...
        ### start of your code.
        x = np.expand_dims(V1[:, :2].flatten(), 1)

        grad_flatten = np.expand_dims(grad.flatten(), 1)

        alpha = backtracking_line_search(-grad_flatten, grad_flatten, x, theta, beta, c,
                                         compute_optimization_objective, *[F, E, x_csl, L0, w])

        V1[:, :2] = V1[:, :2] - alpha * grad
        ### end of your code.
